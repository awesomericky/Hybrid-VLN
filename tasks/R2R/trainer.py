...
class PanoSeq2SeqTrainer():
    ...
    def train(self, epoch, train_env, tb_logger=None):
        ...
        for iter in range(1, self.train_iters_epoch + 1):
            ...
            if iter % 10 == 0:
                ...
                wandb.log({'train/dist_from_goal':dist_from_goal})
        ...
        wandb.log({'epoch/train/dist_from_goal': dists.avg})
        wandb.log({'epoch/train/movements': movements.avg})
        if self.agent.value_loss is not None:
    ...
    def eval(self, epoch, val_env, tb_logger=None):
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if iter % 5 == 0:
                    ...
                    wandb.log({'val/{}/loss'.format(env_name): loss})
                    wandb.log({'val/{}/dist_from_goal'.format(env_name): dist_from_goal})
        ...
        wandb.log({'val/epoch/{}/movements'.format(env_name): movements.avg})
        if self.agent.value_loss is not None:
